...
confirmation "Do you want to install must-have packages?" &&
    sudo pacman -S --needed git zip unzip make fzf less iwd neofetch pulseaudio ripgrep vim wget xclip xdg-utils xorg make base-devel
...
confirmation "Do you want to install personal config packages?" && {
    sudo pacman -S --needed bspwm sxhkd polybar alacritty dunst pavucontrol rofi ranger neovim picom feh flameshot
    ...
    confirmation "Do you want to install zsh?" && {
        ...
        confirmation "Do you want to change your default shell to ZSH?" &&
        chsh -s /usr/bin/zsh
